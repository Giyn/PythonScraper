...
POSITIONS = ['530', '538', '763', '765', '531', '736', '854', '801', '600', '613', '599', '635',
             '702', '703', '653', '639', '636', '654', '551', '719', '749', '681', '682', '622']
INDEX_URL = 'https://sou.zhaopin.com/?jl={position}&kw={keyword}&p={page}'
...
async def scrape_index(position, keyword, page=1):
    """构造 URL 并爬取
    ...
    """
    url = INDEX_URL.format(position=position, keyword=keyword, page=page)
...
async def parse_index():
    ...
    """
    return await tab.xpath("//div[@class='joblist-box__item clearfix']/a")
...
async def parse_detail():
    ...
    try:
        job_name = detail_doc.xpath("//h3[@class='summary-plane__title']/text()")[0]
    except:
        job_name = ''
    try:
        academic_requirements = \
        detail_doc.xpath("//ul[@class='summary-plane__info']/li[4]/text()")[0]
    except:
        academic_requirements = ''
    try:
        salary = detail_doc.xpath("//span[@class='summary-plane__salary']/text()")[0].strip()
    except:
        salary = ''
    try:
        description = ''.join(''.join(
            detail_doc.xpath("//div[@class='describtion__detail-content']//text()")).split())
    except:
        description = ''
    try:
        company_name = detail_doc.xpath("//a[@class='company__title']/text()")[0].strip()
    except:
        company_name = ''
    try:
        work_place = detail_doc.xpath("//ul[@class='summary-plane__info']/li[1]/a/text()")[0]
    except:
        work_place = ''
        ...
        for position in POSITIONS:
            for key_word in key_words_list:
                ...
                for page in range(1, 11):
                    try:
                        await scrape_index(position, key_word, page)
